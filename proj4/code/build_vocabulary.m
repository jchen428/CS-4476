...
m = 20; % m = 50 -> 0.674 -> 0.678, insignificant

featSamples = zeros(128, n * m);
for i = 1 : n
    % Read image and get SIFT features
    image = imread(image_paths{i});
    [~, features] = vl_dsift(single(image), 'step', 5); % 5 -> 0.699, 10 -> 0.673
    
    % If a ton of features, randomly sample m features and store them in featSamples
    if size(features, 2) > m
        featSamples( : , m * (i - 1) + 1 : m * i) = features( : , randsample(size(features, 2), m));
    end
end

% Cluster SIFT features with K-means sampling
[vocab, ~] = vl_kmeans(single(featSamples), vocab_size);
